...
/*   Updated: 2024/03/04 11:28:59 by aelbouab         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	map_control(char *maps, char **lines, int l)
{
	...
	while (maps[i])
	{
		if ((maps[i] == '\n' && maps[i + 1] == '\n')
			|| maps[0] == '\n' || maps[ls - 1] == '\n')
			write_err("don't include empty lines");
		if (maps[i] != 'P' && maps[i] != 'E'
			&& maps[i] != 'C' && maps[i] != '1'
			&& maps[i] != '0' && maps[i] != '\n')
			write_err("there is some unknown chars");
		i++;
	}
}
...
static void	wall_error(char **lines)
{
	...
	{
		if (lines[0][i] != '1')
			write_err("Map must be enclosed by walls");
		i++;
	}
	i = 0;
	ls = line_len(lines[i]);
...
static void	items_error(char **lines, int p, int e, int c)
{
	...
	if (p != 1 || e != 1 || c < 1)
		write_err("you need 1P & 1E & at less 1C");
}
